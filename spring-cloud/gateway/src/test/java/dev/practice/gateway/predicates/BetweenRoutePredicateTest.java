...
@ActiveProfiles("predicate-between")
...
public class BetweenRoutePredicateTest {
    ...
    @DisplayName("Spring cloud gateway 에서 기본으로 제공해주는 between predicate")
    @Test
    void test() {
         ...
         * 2. gateway 서버에서는 현재 테스트 날짜가 2023.12.25 이므로 predicate between 조건에 의해 between-december(id) 가 동작
         ...
         * 5. gateway 의 AddResponseHeader filter 에 의해 header key : X-Test, header value : between-december 가 추가되어 최종 webTestClient 로 응답이 내려간다.
         * 6. 검증
         */
        ...
        webTestClient.get()
                ...
                .exchange()
                .expectStatus().isOk()
                .expectHeader().valueEquals("X-Test", "between-december")
