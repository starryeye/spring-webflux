...
public class MyPredicateRoutePredicateFactory extends AbstractRoutePredicateFactory<MyPredicateRoutePredicateFactory.Config> {
     ...
     * - AbstractRoutePredicateFactory 를 상속하는 클래스 이름은 predicate name 을 앞쪽에 명시해야한다. ("MyPredicate"RoutePredicateFactory)
     *      XXXRoutePredicateFactory 를 바탕으로 predicate name 을 매칭한다. (MyPredicate 이 predicate name)
    ...
    public MyPredicateRoutePredicateFactory() { // 필수
        super(Config.class);
    }
    ...
    public List<String> shortcutFieldOrder() { // shortcut 방식으로 작성 지원용
        return List.of("greeting"); // predicate 에서 MyPredicate=~~~~ 으로 작성할 때.. ~~~~ 에 들어갈 값의 key 에 해당 (fully expanded predicate args 에 해당)
    }
